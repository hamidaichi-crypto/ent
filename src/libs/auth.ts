            ...
            async authorize(credentials) {
                try {
                    ...
                    if (!response.ok) {
                        ...
                        throw new Error(JSON.stringify({ message: errorData.message || `HTTP error! status: ${response.status}`, code: errorData.error_code }));
                    }
                    ...
                    throw new Error(error.message || 'Failed to authenticate with the external API.');
                }
            }
        }),
